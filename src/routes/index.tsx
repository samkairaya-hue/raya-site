  ...
  return (
    <section className="section-padded-mobile" style={{ background: "var(--bg-cream)", padding: "110px 0" }}>
      <div className="container-x">
        ...
        <div className="grid gap-5 mini-card-grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))" }}>
          {data.outcomes.map((c) => {
            const clickable = !!c.target_body?.trim();
            const inner = (
              <div className="card-surface h-full flex flex-col items-center text-center mini-card" style={{ padding: "36px 20px" }}>
                <div className="mb-4 flex items-center justify-center mini-card-icon" style={{ width: 48, height: 48, borderRadius: "50%", background: "color-mix(in oklab, var(--accent-primary) 12%, transparent)", color: "var(--accent-primary)" }}>
                ...
                <h3 className="mini-card-title" style={{ fontSize: "1.05rem", marginBottom: 8 }}>{c.title}</h3>
                <p className="mini-card-desc" style={{ fontSize: ".92rem", lineHeight: 1.6 }}>{c.description}</p>
              </div>
            );
            return clickable ? (
